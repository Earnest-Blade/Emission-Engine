...
namespace Emission.Shading
{
    public class Shader : IDisposable
    {
        ...
        protected static string[] SplitShader(string path)
        {
            ...
            var type = ShaderParseType.None;

            void WriteToShader(string line, ShaderParseType shaderType)
            {
                switch (shaderType)
                {
                    // Add line to vertex content at array position
                    case ShaderParseType.Vertex:
                        content[0] += line + "\n";
                        break;
                    case ShaderParseType.Fragment:
                        content[1] += line + "\n";
                        break;
                    case ShaderParseType.Define:
                        content[2] += line + "\n";
                        break;
                }
            }
            ...
            foreach(string line in lines)
            {
                if (line.Contains(":") && !line.StartsWith("//"))
                {
                    ...
                    if (line.Contains("vertex:")) type = ShaderParseType.Vertex;

                    // Define type for fragment shader
                    else if (line.Contains("fragment:")) type = ShaderParseType.Fragment;
                    
                    // Define header statement
                    else if (line.Contains("define:")) type = ShaderParseType.Define;
                }
                else if (line.Contains("#"))
                {
                    ...
                    switch (data[0])
                    {
                        ...
                        case "include":
                            ...
                            content[2] += include[2] + '\n';
                            WriteToShader(type == ShaderParseType.Vertex ? include[0] : include[1], type);
                            break;
