...
namespace Emission.Core.Mathematics
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        public bool IsIdentity => X == 0.0f && Y == 0.0f && Z == 0.0f && W == 0.0f;
        ...
        public readonly float Angle
        {
            get
            {
                ...
                if (length < MathHelper.EPSILON) return 0.0f;
        ...
        public readonly Vector3 Axis
        {
            ...
            {
                float length = X * X + Y * Y + Z * Z;
                if(length < MathHelper.EPSILON) return Vector3.UnitX;
        ...
        public Vector3 ToEulerAngles()
        {
            ...
            euler.Y = MathF.Abs(fSinPitch) >= 1 ? MathF.CopySign(MathHelper.PI / 2, fSinPitch) : MathF.Asin(fSinPitch);
        ...
        public static Quaternion Multiply(Quaternion left, Quaternion right)
        {
            ...
            float cx = ly * rz - lz * ry;
            float cy = lz * rx - lx * rz;
            float cz = lx * ry - ly * rx;

            float dot = lx * rx + ly * ry + lz * rz;
        ...
        {
            if (value.LengthSquared > MathHelper.EPSILON)
            {
        ...
        {
            if (value.Length > MathHelper.EPSILON)
            {
        ...
        public static Quaternion FromEulerAngles(Vector3 vector) => FromEulerAngles(vector.X, vector.Y, vector.Z);
        public static Quaternion FromEulerAngles(float pitch, float yaw, float roll)
        {
            var cosP = MathF.Cos(pitch * 0.5f);
            var cosY = MathF.Cos(yaw * 0.5f);
            var cosR = MathF.Cos(roll * 0.5f);
            var sinP = MathF.Sin(pitch * 0.5f);
            var sinY = MathF.Sin(yaw * 0.5f);
            var sinR = MathF.Sin(roll * 0.5f);
        ...
        public static Quaternion Lerp(Quaternion left, Quaternion right, float amount)
        {
            ...
            {
                q.X = t1 * left.X + t * right.X;
                q.Y = t1 * left.Y + t * right.Y;
                q.Z = t1 * left.Z + t * right.Z;
                q.W = t1 * left.W + t * right.W;
            }
            else
            {
                q.X = t1 * left.X - t * right.X;
        ...
        {
            return (MathF.Abs(other.X - X) < MathHelper.EPSILON
                    && MathF.Abs(other.Y - Y) < MathHelper.EPSILON
                    && MathF.Abs(other.Z - Z) < MathHelper.EPSILON
                    && MathF.Abs(other.W - W) < MathHelper.EPSILON);
        }
