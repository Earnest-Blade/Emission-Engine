...
namespace Emission.Natives.GLFW
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate void MouseButtonCallback(IntPtr window, int button, InputState state, ModifierKeys modifiers);
    ...
    /// <param name="mods">	Bit field describing which modifier keys were held down.</param>
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate void KeyCallback(IntPtr window, int key, int scanCode, InputState state, ModifierKeys mods);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate void JoystickCallback(int joystick, int status);
